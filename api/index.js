...
const userController = require('./Controllers/userController');
const deleteController = require('./Controllers/deleteController');
>>>>>>> e66a60aefbb4de522830df3668120be15dd3b444

const app = express();
app.use(express.json());
app.use('/uploads', express.static(path.join('/tmp', 'uploads'))); // Use /tmp for static files
app.use(cookieParser());
app.use(cors({
    credentials: true,
    methods: 'GET,HEAD,PUT,PATCH,POST,DELETE',
...
app.post("/upload-by-link", async (req, res) => {
    const { link } = req.body;
    const newName = "photo" + Date.now() + ".jpg";
    const destination = path.join('/tmp', newName); // Use /tmp directory
    await imageDownloader.image({
        url: link,
        dest: destination,
    });
...
const photosMiddleware = multer({ dest: "/tmp/uploads" }); // Use /tmp directory
app.post("/upload", photosMiddleware.array("photos", 100), (req, res) => {
    const uploadedFiles = [];
    for (let i = 0; i < req.files.length; i++) {
        const { path, originalname } = req.files[i];
